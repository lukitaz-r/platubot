...
export default {
  name: 'palubi-temporada',
  aliases: ['paltemporada', 'temporadapal', 'nuevatemporadapal'],
  desc: 'Crea nueva temporada de Platubi con inscripción y playoffs automáticos',
  permisos: ['Administrator'],
  run: async (client, message, args) => {
    const embed = new EmbedBuilder()
      .setTitle('🏆 Nueva Temporada — Palubi')
      .setDescription(
    ...
    collector.on('end', async (collected, reason) => {
      ...
      try {
        ...
        for (const j of inscritos) {
          await registrarJugadorGlobal(j.nombre, j.id, 'Palubi');
        }

        // 2. Crear nueva temporada
        const liga = await Segunda.create({ fechaDeInicio: new Date() });
        ...
        resumen += `Liga lista para generar fixture con \`!palubi-gestion\`.`;

        await loading.edit(resumen);

      } catch (error) {
        console.error('Error en palubi-temporada:', error);
